...
data0 <- enframe(read_lines("ca_sdcel_experiment.txt"), value = "line") |>
...
W = 8
H = 6
ggsave(paste0("CA3_KdtreeVsQuadtree_Creation.pdf"), width = W, height = H)
...
p = ggplot(dataPartitioning, aes(x = as.factor(partitions), y = time, group = tree)) +
  ...
  theme_bw()
plot(p) 
ggsave(paste0("CA3_KdtreeVsQuadtree_Partitioning.pdf"), width = W, height = H)
...
p = ggplot(dataOverlay, aes(x = as.factor(partitions), y = time, group = tree)) +
  ...
  theme_bw()
plot(p) 
ggsave(paste0("CA3_KdtreeVsQuadtree_Overlay.pdf"), width = W, height = H)

## Space plot
dataSpace <- enframe(read_lines("ca_sdcel_experiment.txt"), value = "line") |>
...
ggsave(paste0("CA3_KdtreeVsQuadtree_Space.pdf"), width = W, height = H)
