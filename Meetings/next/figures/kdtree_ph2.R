...
data0 <- enframe(read_lines("kdtree_ph2.txt"), value = "line") |>
...
W = 8
H = 6
ggsave(paste0("PH2_KdtreeVsQuadtree_Creation_time.pdf"), width = W, height = H)
...
p = ggplot(dataOverlayTime, aes(x = as.factor(partitions), y = time, group = tree)) +
  ...
  theme_bw()
plot(p) 
ggsave(paste0("PH2_KdtreeVsQuadtree_Overlay_time.pdf"), width = W, height = H)
...
ggsave(paste0("PH2_KdtreeVsQuadtree_space.pdf"), width = W, height = H)