...
W = 8
H = 6
ggsave(paste0("US_KdtreeVsQuadtree_Creation.pdf"), width = W, height = H)
...
p = ggplot(dataPartitioning, aes(x = as.factor(partitions), y = time, group = tree)) +
  ...
  theme_bw()
plot(p) 
ggsave(paste0("US_KdtreeVsQuadtree_Partitioning.pdf"), width = W, height = H)
...
p = ggplot(dataOverlay, aes(x = as.factor(partitions), y = time, group = tree)) +
  ...
  theme_bw()
plot(p) 
ggsave(paste0("US_KdtreeVsQuadtree_Overlay.pdf"), width = W, height = H)
...
ggsave(paste0("US_KdtreeVsQuadtree_Space.pdf"), width = W, height = H)
