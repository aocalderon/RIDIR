...
for N in {1..5}; do
    for i in ${!SS[@]}; do
	echo "scala -cp $LIBS:$JAR $CLASS --input1 ${THE_PATH}/${THE_DIR}/A$i.wkt --input2 ${THE_PATH}/${THE_DIR}/B$i.wkt --tag1 $THE_DIR --tag2 $i --appid $N --perc $THE_DIR"
	scala -cp $LIBS:$JAR $CLASS --input1 ${THE_PATH}/${THE_DIR}/A$i.wkt --input2 ${THE_PATH}/${THE_DIR}/B$i.wkt --tag1 $THE_DIR --tag2 $i --appid $N --perc $THE_DIR
    done
done
