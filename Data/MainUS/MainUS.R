...
p = ggplot(data3, aes(x = partitions, y = time, fill = stage)) + 
  ...
  labs(x="Number of partitions", y="Time [s]", title=paste0("Performance census tracts dataset (36M edges) for SDCEL computation"))
plot(p)

ggsave(paste0("MainUS.pdf"), width = 8, height = 5)