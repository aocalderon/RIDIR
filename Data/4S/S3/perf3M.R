library(tidyverse)
setwd("~/RIDIR/Data/4S/S3")

data0 = enframe(read_lines("perf3M.txt"), value="line")
...
write_tsv(data3, "perf3M.tsv")

p = ggplot(data3, aes(x = partitions, y = time, fill = stage)) + 
  geom_col(width = 0.7, position="dodge") + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(x="Number of partitions", y="Time [s]", title=paste0("Performance census dataset (3.75M edges) for SDCEL computation"))
plot(p)

ggsave(paste0("perf3M.pdf"), width = 8, height = 5)