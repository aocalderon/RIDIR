...
data0a = enframe(read_lines("OR_Overlay_Test_v01.txt"), value="line")
data0b = enframe(read_lines("OR_Overlay_Test_v02.txt"), value="line")
...
data2 = data1 %>% 
  ...
  mutate(time = as.numeric(time) / 1000.0, overlay_level = as.numeric(overlay_level)) 
...
write_tsv(data3, "OR.tsv") 

p = ggplot(data3, aes(x = method, y = time)) + 
  geom_col(width = 0.7, position="dodge") + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(x="Method for overlay", y="Time [s]", title=paste0("Performance Oregon census tracts dataset (3.5M edges) for SDCEL computation"))
plot(p)

ggsave(paste0("OR.pdf"), width = 8, height = 5)
