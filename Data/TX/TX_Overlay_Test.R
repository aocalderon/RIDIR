...
data0 = enframe(read_lines("TX_Overlay_Test_v01.txt"), value="line")
...
write_tsv(data3, "TX.tsv") 

p = ggplot(data3, aes(x = method, y = time)) + 
  geom_col(width = 0.7, position="dodge") + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(x="Method for overlay", y="Time [s]", title=paste0("Performance TX census tracts dataset for SDCEL computation"))
plot(p)

ggsave(paste0("TX.pdf"), width = 8, height = 5)
