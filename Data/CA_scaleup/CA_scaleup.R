...
data0 = enframe(read_lines("CA_scaleup_v03.txt"), value="line")

data1 = data0 %>%
  filter(str_detect(line, 'TIME')) 

fields1 = c("ts","start","appId","time","tag","stage","data")
fields2 = c("CA", "dataset","tolerance", "overlay_method", "overlay_level", "partitions", "nodes", "run")
fields3 = c(NA, "size")
data2 = data1 %>% 
...
write_tsv(data3, "CA_scaleup.tsv")

p = ggplot(data3, aes(x = size, y = time)) + 
  geom_col(width = 0.7, position="dodge") + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(x="Size [number of edges]", y="Time [s]", title=paste0("Scale up for CA dataset")) +
  facet_wrap(~ stage)
plot(p)

ggsave(paste0("CA_scaleup.pdf"), width = 4, height = 2.5)